...
class Mixpanel extends Base_MixpanelBase
{
    ...
    public function __construct($token, $options = array())
    {
        ...
        $people = $this->setupProducer($token, $options, 'people', 'Producers_MixpanelPeople');
        $this->setPeopleProducer($people);

        $people = $this->setupProducer($token, $options, 'events', 'Producers_MixpanelEvents');
        $this->setEventsProducer($people);
    }

    /**
     * @param string $token
     * @param array  $options
     * @param string $producerAttr
     * @param string $defaultClassName
     *
     * @return Producers_MixpanelBaseProducer
     */
    protected function setupProducer($token, array $options, $producerAttr, $defaultClassName)
    {
        if (isset($options['producers'][$producerAttr])) {
            if ($options['producers'][$producerAttr] instanceof Producers_MixpanelBaseProducer) {
                $producer = $options['producers'][$producerAttr];
            } elseif (is_string($options['producers'][$producerAttr])) {
                $className = $options['producers'][$producerAttr];
                $producer = new $className($token, $options);
            } else {
                throw new \InvalidArgumentException(
                    sprintf('Invalid "%s" producer instance/class-name', $producerAttr)
                );
            }
        } else {
            $producer = new $defaultClassName($token, $options);
        }
    ...
    /**
     * @param \Producers_MixpanelBaseProducer $producer
     *
     * @return self
     */
    public function setPeopleProducer(Producers_MixpanelBaseProducer $producer)
    {
        $this->people = $producer;
     ...
     * @return self
     */
    public function setEventsProducer(Producers_MixpanelBaseProducer $producer)
    {
        $this->events = $producer;
     ...
     */
    public function getPeopleProducer()
    {
        return $this->people;
    }
    ...
    {
        return $this->events->flush($desired_batch_size);
    }
    ...
    {
        $this->events->createAlias($original_id, $new_id);
    }
}
